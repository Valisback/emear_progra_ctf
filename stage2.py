...
if netRetrieved:
    ...
    if response.status_code == 200:
        ...
        for device in resp:
            ...
            data['devices_meraki'].append(elem)

else:
    print("Problem retrieving network")

## Getting devices from DNA - Center: 

base_url_dna = config["DNAC_BASE_URL"]
auth_endpoint = "system/api/v1/auth/token"

user = config["DNAC_USER"]
...
auth_response = requests.post(
    url=f"{base_url_dna}{auth_endpoint}", auth=(user, password), headers = headers).json()
...
endpoint_url = "intent/api/v1/network-device"
global_url = f"{base_url_dna}{endpoint_url}"
    ...
    if response.status_code == 200:
        ...
        for device in devices:
            ...
            if ("hostname" in device):
                elem["name"] = device["hostname"]
            if ("serialNumber" in device):
                elem["serial"] = device["serialNumber"]
            if ("macAddress" in device):
                elem["mac"] = device["macAddress"]
            if ("type" in device):
                elem["model"] = device["type"]  
            data['devices_DNAC'].append(elem)
